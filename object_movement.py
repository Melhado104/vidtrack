...
# define lower/upper boundaries of the "bird" in the HSV color space,
# the list of tracked points
# greenLower RGB = 29, 86, 6 and greenUpper RGB = 64, 255, 255
# As I am using yellow bird = YellowLower RGB = 233, 240, 24
# for other colors see: RGB codes chart
greenLower = (29, 86, 6)
...
while True:
    ...
    # if we are viewing a video and and we did not grab a frame, then we have
    ...
    # find contours in the mask and initialize the currentt (x,y) center of
    # ball/bird
    ...
    # loop over th set of tracked points
    for i in np.arange(1, len(pts)):
        ...
        if counter >= 10 and i == 1 and pts[-10] is not None:
            # compute the diff btw the x and y coord and re-initialize
            # the direction text variables
            dX = pts[-10][0] - pts[i][0]
            dY = pts[-10][1] - pts[i][1]
            (dirX, dirY) = ("", "")

            # ensure there is significant movement in the x-direction
            # mais sensibilidade: diminui o 20 (para objetos pequenos)
            if np.abs(dX) > 20:
...
# cleanup the camera and close any open winndows
camera.release()
